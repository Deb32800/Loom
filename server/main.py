...
async def websocket_endpoint(
    ...
    """
    # Generate a client_id if none provided
    if not client_id:
        client_id = f"user_{uuid.uuid4().hex[:8]}"

    # ── Step 1: Accept connection and register client ─────────
    client = await session_manager.connect(client_id, websocket)
    logger.info(f"New connection: {client_id}")

    try:
        # ── Step 2: Send current document state to the new client ─
        # This is the "sync" message. The client uses this to
        # initialize their editor with the current content and
        # set their local revision number.
        await session_manager.send_to(client_id, {
            "type": "sync",
            "content": document.content,
            "revision": document.revision,
            "document_id": document.document_id,
            "clients": session_manager.get_client_list(),
            "your_client_id": client_id,
            "your_color": client.color
        })

        # ── Step 3: Tell everyone else that someone joined ────
        ...
        # ── Step 4: Message loop ──────────────────────────────
        # This loop runs for the entire lifetime of the connection.
        # It waits for a message, processes it, and repeats.
        # When the client disconnects, receive_json() raises
        # WebSocketDisconnect and we break out.
        while True:
...
async def _handle_operation(client_id: str, data: dict) -> None:
    ...
    try:
        ...
        # Apply to document (transforms against history if client is behind)
        result = document.receive_operation(
            op=op,
        ...
        if result is None:
            # Operation was a no-op after transform — nothing to broadcast
            # Still send an ack so the client knows to move on
            await session_manager.send_to(client_id, {
                "type": "ack",
                "revision": document.revision
            })
        ...
        # The client needs this to move from "AwaitingConfirm" state
        # back to "Synchronized" in its client-side OT state machine.
        await session_manager.send_to(client_id, {
            "type": "ack",
        ...
        # We exclude the sender because they already applied it locally.
        await session_manager.broadcast(
            {
                "type": "operation",
                "op": serialize_operation(result.op),
                "revision": result.revision,
                "client_id": client_id
            },
            exclude_client=client_id
        ...
        logger.info(
            f"Op from {client_id}: {serialize_operation(result.op)} "
...
async def _handle_cursor(client_id: str, data: dict) -> None:
    ...
    # Get this client's color for the cursor display
    client_list = session_manager.get_client_list()
    client_info = next(
        (c for c in client_list if c["client_id"] == client_id), None
    )
...
# ── Static file serving (for the frontend in Phase 5) ────────
# This will serve the HTML/CSS/JS files from the client/ directory.
# For now, we just set up the mount point.
...
if os.path.isdir(CLIENT_DIR):
    ...
    async def root():
        ...
        return {
            ...
            "connected_clients": session_manager.client_count
        }
